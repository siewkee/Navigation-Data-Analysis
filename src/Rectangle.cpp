...
int Rectangle::get_numOfVertices()
{
	return this->noOfVertices;
}


void Rectangle::addVertices_toArray(Points p)
{
    this->onshape_points.push_back(p);
}

Points Rectangle::start_Point()
{
...
Points Rectangle::draw_verti_upDown(Points draw, string direction)
{
...
Points Rectangle::draw_horiz_rightLeft(Points draw, string direction)
{
...
string Rectangle::toString()
{
	...
	for(Points p : onshape_points)
	{
		if (p.pos == Vertices)
		{
			rec += "Point [" + to_string(counter) + "] : (" + to_string(p.x) + ", " + to_string(p.y) + ") \n";
			counter++;
		}
	}
	...
	{
		if (p.pos == Perimeter)
			rec += "(" + to_string(p.x) + ", " + to_string(p.y) + ")  ";
